package com.YiDian.RainBow.topic.adapter;
...
public class TopicAdapter extends RecyclerView.Adapter<TopicDynamicViewHolder> {
    private final Activity context;
    private final List<TopicBean.ObjectBean.ListBean> list;
    ...
    int userid;
    private TopicBean.ObjectBean.ListBean listBean;
    private TopicBean.ObjectBean.ListBean.UserInfoBean userInfo;
    private int id;
    private TopicDynamicViewHolder viewHolder;



    public TopicAdapter(Activity context, List<TopicBean.ObjectBean.ListBean> list,Tencent tencent) {
    ...
    @NonNull
    @Override
    public TopicDynamicViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        //纯文本
        if (viewType == 1) {
            viewHolder = TopicDynamicViewHolder.createViewHolder(context, parent, R.layout.item_dynamic_text);
            return viewHolder;
        }
        //纯图片
        if (viewType == 2) {
            viewHolder = TopicDynamicViewHolder.createViewHolder(context, parent, R.layout.item_dynamic_img);
            return viewHolder;
        }
        //文本加图片
        if (viewType == 21) {
            viewHolder = TopicDynamicViewHolder.createViewHolder(context, parent, R.layout.item_dynamic_text_img);
            return viewHolder;
        }
        //纯视频
        if (viewType == 3) {
            viewHolder = TopicDynamicViewHolder.createViewHolder(context, parent, R.layout.item_dynamic_video);
            return viewHolder;
        }
        //视频加文本
        if (viewType == 31) {
            viewHolder = TopicDynamicViewHolder.createViewHolder(context, parent, R.layout.item_dynamic_video_text);
            return viewHolder;
        }
        return null;
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull TopicDynamicViewHolder holder, int position) {
